...
function ColorPickers(props) {
  ...
  const content = props.name.map(name => {
    ...
    return (
      <div key={name.key} className="entire-picker-class">
        <div
          className="color-picker-class"
          key={name.key}
          style={{ backgroundColor: temp, marginTop: "1rem" }}
        >
          <SketchPicker
            disableAlpha={true}
            className="sketch"
            color={getFruit.tempcolor.hex}
            key={name.key}
            data-tag={name.key}
            onChange={color => {
              //  console.log(color)
              //  console.log(getFruit+"fruit")
              props.onChangeComplete(color, name.key);
            }}
          ></SketchPicker>
        </div>
        <button onClick={() => props.onDeleteClick(name.key)}>
          <span>✖</span>
        </button>
      </div>
    );
